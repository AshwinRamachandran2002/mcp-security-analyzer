                ...
                {endpoints.map((endpoint) => (
                  <Card key={endpoint.endpoint_id}>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3 min-w-0 flex-1">
                          ...
                          <div className="min-w-0 flex-1">
                            <CardTitle className="text-lg truncate">{endpoint.hostname}</CardTitle>
                            <p className="text-sm text-muted-foreground">
                              <span className="hidden sm:inline">{endpoint.server_count} servers (</span>
                              <span className="sm:hidden">{endpoint.server_count} • </span>
                              {endpoint.stdio_count} STDIO<span className="hidden sm:inline">,</span>
                              <span className="sm:hidden"> • </span> {endpoint.http_count} Remote
                    ...
                    {expandedEndpoints.has(endpoint.endpoint_id) && (
                      <CardContent className="pt-0 pb-4 px-6 bg-gray-50/30 dark:bg-gray-900/30">
...
function EndpointServerView({ endpoint }: { endpoint: EndpointData }) {
  ...
  return (
    <div>
      ...
      <div className="flex border-b border-gray-200 dark:border-gray-700 mb-6 -mx-6 px-6 overflow-x-auto">
        <button
        ...
        >
          <Database className="inline-block w-4 h-4 mr-1 sm:mr-2" />
          <span className="hidden xs:inline">STDIO </span>
          ({endpoint.stdio_count})
        </button>
        <button
        ...
        >
          <Wifi className="inline-block w-4 h-4 mr-1 sm:mr-2" />
      ...
      <div className="space-y-3">
        {endpoint.servers
          ...
          .map((server, index) => (
            <div key={`${server.name}-${index}`} className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
              <Link
                href={`/servers/${server.name}/dynamic`}
                className="block p-4 flex flex-col md:flex-row md:items-center md:justify-between hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-150 rounded-lg"
              >
                      <div className="mb-3 md:mb-0 flex-1 min-w-0">
                        <div className="text-lg sm:text-xl font-semibold flex items-center flex-wrap gap-2 mb-1">
                          {getTransportIcon(server.transport)}
                          <span className="ml-2 truncate">{server.name}</span>
                          {getStatusIcon(server.status)}
                        </div>
                        <div className="flex items-center flex-wrap gap-2 mb-2">
                          {server.server_classification && getClassificationBadge(server.server_classification)}
                      ...
                      <div className="flex items-center space-x-3 sm:space-x-4 md:space-x-6 flex-shrink-0">
                        <div className="text-center">
                          <div className="text-xl sm:text-2xl font-bold">{server.function_count}</div>
